...
locals {
  ...
  # Staging uses staging.koumpa.com as base
  staging_domain    = "staging.${var.domain_name}"
  wildcard_domain   = "*.staging.${var.domain_name}"
...
module "storage" {
  ...
  name_prefix         = local.name_prefix
  environment         = var.environment
  # Wildcard for user apps + base staging domain for landing page
  domain_aliases      = [local.wildcard_domain, local.staging_domain]
...
module "api" {
  ...
  # CloudFront
  cloudfront_domain = local.staging_domain
...
# CloudFront alias record for staging landing page (staging.koumpa.com)
...
# CloudFront wildcard record for user apps (*.staging.koumpa.com)
